...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const DFLOAT* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(DFLOAT* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(DFLOAT s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(DFLOAT s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const DFLOAT &angle);
  static Matrix rotMatY(const DFLOAT &angle);
  static Matrix rotMatZ(const DFLOAT &angle);
  ...
  Matrix  operator* (const DFLOAT &s);  // multiply with scalar
  Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix  operator/ (const DFLOAT &s);  // divide by scalar
  Matrix  operator- ();                // negative matrix
  Matrix  operator~ ();                // transpose
  DFLOAT   l2norm ();                   // euclidean norm (vectors) / frobenius norm (matrices)
  DFLOAT   mean ();                     // mean of all elements in matrix
  ...
  DFLOAT  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M,DFLOAT eps=1e-20);            // solve linear system M*x=B, replaces *this and M
  bool   lu(int32_t *idx, DFLOAT &d, DFLOAT eps=1e-20);        // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory ();
  inline DFLOAT pythag(DFLOAT a,DFLOAT b);
