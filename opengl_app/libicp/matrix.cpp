...
static DFLOAT sqrarg;
#define SQR(a) ((sqrarg=(a)) == 0.0 ? 0.0 : sqrarg*sqrarg)
static DFLOAT maxarg1,maxarg2;
...
Matrix::Matrix (const int32_t m_,const int32_t n_,const DFLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Matrix::Matrix (const Matrix &M) {
  ...
  for (int32_t i=0; i<M.m; i++)
    memcpy(val[i],M.val[i],M.n*sizeof(DFLOAT));
}
...
void Matrix::getData(DFLOAT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(DFLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(DFLOAT s,int32_t i1,int32_t i2) {
  if (i2==-1) i2 = fmin(m-1,n-1);
  for (int32_t i=i1; i<=i2; i++)
    val[i][i] = s;
}
...
Matrix Matrix::rotMatX (const DFLOAT &angle) {
  DFLOAT s = sin(angle);
  DFLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const DFLOAT &angle) {
  DFLOAT s = sin(angle);
  DFLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const DFLOAT &angle) {
  DFLOAT s = sin(angle);
  DFLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::operator* (const DFLOAT &s) {
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
bool Matrix::solve (const Matrix &M, DFLOAT eps) {
...
bool Matrix::lu(int32_t *idx, DFLOAT &d, DFLOAT eps) {
  ...
  int32_t i,imax,j,k;
  DFLOAT   big,dum,sum,temp;
  DFLOAT* vv = (DFLOAT*)malloc(n*sizeof(DFLOAT)); // vv stores the implicit scaling of each row.
  d = 1.0;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  DFLOAT* w   = (DFLOAT*)malloc(n*sizeof(DFLOAT));
  DFLOAT* rv1 = (DFLOAT*)malloc(n*sizeof(DFLOAT));

  int32_t flag,i,its,j,jj,k,l,nm;
  DFLOAT   anorm,c,f,g,h,s,scale,x,y,z;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((DFLOAT)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((DFLOAT)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        ...
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
          if ((DFLOAT)(fabs(f)+anorm) == anorm) break;
          g = w[i];
          h = pythag(f,g);
          w[i] = h;
          h = 1.0/h;
  ...
  int32_t s2,inc=1;
  DFLOAT   sw;
  DFLOAT* su = (DFLOAT*)malloc(m*sizeof(DFLOAT));
  DFLOAT* sv = (DFLOAT*)malloc(n*sizeof(DFLOAT));
  ...
  W = Matrix(fmin(m,n),1,w);
  
  // extract mxm submatrix U
  U2.setMat(U.getMat(0,0,m-1,fmin(m-1,n-1)),0,0);
...
void Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  ...
  val    = (DFLOAT**)malloc(m*sizeof(DFLOAT*));
  val[0] = (DFLOAT*)calloc(m*n,sizeof(DFLOAT));
...
DFLOAT Matrix::pythag(DFLOAT a,DFLOAT b) {
  DFLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
